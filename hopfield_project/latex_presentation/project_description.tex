...
\begin{itemize}
    \item \textbf{Dense Associative Memory} (Krotov \& Hopfield, 2016): Higher-order interactions that sharpen energy minima.
    \item \textbf{Modern Hopfield Networks} (Ramsauer et al., 2020): Show that Hopfield-style retrieval is mathematically equivalent to attention mechanisms in transformers \cite{modern_hopfield}.
    \item \textbf{Huge-Capacity Associative Memory} (Demircigil et al., 2017): Introduces an energy-based model with exponentially many stable memories \cite{demircigil2017}.
\end{itemize}

In this project we implement and experiment with the \emph{classical} Hebbian Hopfield network and a pseudo-inverse variant. We discuss these modern extensions qualitatively in the notebooks but do not reproduce their full theoretical models.
...
\subsubsection{Part 1: Clean Geometric and Letter Patterns}
\begin{itemize}
    \item \textbf{Data}: Synthetic 10×10 binary patterns ($N=100$) including block letters (A--E) and simple geometric shapes.
    \item \textbf{Exploratory Analysis}: We visualize all stored patterns, compute Hamming distances, and plot a pattern similarity (overlap) matrix to see how correlated patterns deform the energy landscape.
    \item \textbf{Noise-Robustness Experiment}: For each stored pattern and for noise levels
    ...
    \item \textbf{Result}: The classical Hopfield network maintains $>90\%$ accuracy up to roughly $20\%-25\%$ noise and then collapses rapidly, matching the intuitive picture of basins of attraction with finite radius.
\end{itemize}

\subsubsection{Part 2: The Simpsons Challenge (Similar Patterns)}
\begin{itemize}
    \item \textbf{Data}: Five simplified black-and-white Simpsons characters (Homer, Marge, Bart, Lisa, Maggie), each resized to $24\times24$ pixels ($N=576$).
    \item \textbf{Motivation}: These faces share strong structure (round heads, similar silhouettes), so Hebbian learning produces highly correlated weight vectors and overlapping energy valleys.
    \item \textbf{Hebbian vs. Pseudo-Inverse}: We train both a classical Hebbian Hopfield network and a \texttt{PseudoInverseHopfield} implementation from the notebook on the same set of characters.
    \item \textbf{Evaluation}: For each character we add moderate noise and measure retrieval accuracy across multiple trials.
    \item \textbf{Result}: On these highly correlated patterns, Hebbian learning fails completely (empirically $\approx 0\%$ accuracy), while the pseudo-inverse rule (and a simplified modern Hopfield variant) achieve near-perfect retrieval (reported as $\approx 100\%$ accuracy in the notebook summary table).
\end{itemize}

\subsubsection{Capacity and Spurious Attractors}
\begin{itemize}
    \item \textbf{Capacity Curve}: For the $N=100$ letter/geometric patterns we store $P = 1, 2, \ldots, 30$ random patterns and measure retrieval accuracy at fixed noise level. Accuracy remains high up to around $P \approx 13$ and then drops sharply, consistent with the theoretical limit $P_{\max} \approx 0.138N$.
    \item \textbf{Spurious Attractor Search}: Using a helper method \texttt{check\_spurious\_attractors} from the notebook, we initialize the network from many random states and record stable states that are not any of the stored patterns. Visual inspection shows these spurious attractors are mixtures of the original memories, matching the theory of "false memories" near capacity.
    \item \textbf{Energy Trajectories}: For representative runs we plot the Hopfield energy $E(\mathbf{s}^{(t)})$ over asynchronous updates and observe strictly decreasing trajectories until convergence, empirically confirming the energy-minimization picture.
\end{itemize}

\subsection{Modern Connection: Transformers}

In the second notebook we outline how modern Hopfield networks with continuous states and exponential energy functions can be written in the same form as transformer attention \cite{modern_hopfield}. We do not run large-scale experiments here, but we provide small illustrative examples that reuse the same image patterns to show how attention-style updates perform associative retrieval.
...
\begin{enumerate}
    ...
    \item \textbf{Scalability Issues:} The weight matrix grows as $O(N^2)$, making large networks memory-intensive. In our notebooks the largest network has $N = 576$ neurons (24×24 Simpsons faces), but the quadratic scaling would quickly become problematic for much larger $N$; we did not explore sparse connectivity or hierarchical architectures that might improve scaling.
...
Demircigil, M., Heusel, J., Löwe, M., Upgang, S., \& Vermet, F. (2017).
\newblock On a model of associative memory with huge storage capacity.
\newblock \textit{arXiv preprint arXiv:1702.01929}. \url{https://arxiv.org/abs/1702.01929}.
